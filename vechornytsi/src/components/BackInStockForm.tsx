  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (code === "REQUEST_COLLECTION_DISABLED") {
        setStatus({
          kind: "error",
          message: "Notes aren't being taken right now.",
        });
        return;
      }
    ...
    return (
      <div className="back-in-stock-success" role="status" aria-live="polite">
        ...
        <p className="back-in-stock-success-detail">
          {status.alreadySubscribed
            ? `We had your note for ${productName}${variantLabel ? ` (${variantLabel})` : ""}. We will write to ${email} the moment it returns.`
            : `We will write to ${email} the moment ${productName}${variantLabel ? ` (${variantLabel})` : ""} returns.`}
        </p>
      </div>
    );
  }
  ...
  return (
    <form className="back-in-stock-form" onSubmit={handleSubmit} noValidate>
      <div className="back-in-stock-headline">
        <span className="back-in-stock-eyebrow">Currently unavailable</span>
        <p className="back-in-stock-lede">
          Leave your email and we will write to you the moment
          {variantLabel ? ` ${variantLabel.toLowerCase()}` : " it"} returns.
        </p>
      </div>
