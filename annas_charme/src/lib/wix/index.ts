...
import { isUnavailableCartLineItem } from "./kit-inventory";
...
const safePrice = (amount?: string | null): number => {
  const n = Number.parseFloat(amount ?? "");
  return Number.isFinite(n) ? n : 0;
};

const isPhantomLineItem = (item: currentCart.LineItem) =>
  isUnavailableCartLineItem(item);

const reshapeCart = (cart: currentCart.Cart): Cart => {
  const lineItems = (cart.lineItems ?? []).filter(
    (item) => !isPhantomLineItem(item),
  );
  const subtotal = lineItems.reduce(
    (acc, item) => acc + safePrice(item.price?.amount) * (item.quantity ?? 0),
    0
  );
  return {
    id: cart._id!,
    ...
    lines: lineItems.map((item) => {
      ...
      const unitPrice = safePrice(item.price?.amount);
      ...
      return {
        ...
        merchandise: {
          ...
          product: {
            ...
            featuredImage,
            title: item.productName?.original ?? "DIY Bracelet Kit",
          } as any as Product,
          url: `/product/${item.url?.split("/").pop() ?? ""}`,
        },
      };
    }),
    totalQuantity: lineItems.reduce((acc, item) => acc + (item.quantity ?? 0), 0),
  };
};
...
export async function createCheckoutUrl(postFlowUrl: string) {
  const siteOrigin = postFlowUrl.replace(/\/$/, "");
  let cart: currentCart.Cart;
  ...
  try {
    ...
    const created = await elevatedCreateCheckout({
    ...
    checkoutId =
      (created as any)?.checkout?._id ??
      (created as any)?.checkoutId ??
      (created as any)?._id;
  }
  ...
  const { redirectSession } = await redirects.createRedirectSession({
    ...
    callbacks: {
      postFlowUrl: siteOrigin,
      thankYouPageUrl: `${siteOrigin}/?ordered=1`,
    },
  });

  const url = redirectSession?.fullUrl;
  if (!url) {
    throw new Error("Could not start checkout redirect.");
  }
