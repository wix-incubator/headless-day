...
- CLIENT_ID_SOURCE: `wix.config.json` → `"appId"` (public OAuth client id; copy from `wix.config.example.json` after `npm create @wix/new@latest init`). Optional server secret in `.env.local` → `WIX_CLIENT_SECRET` (gitignored). Site id: `wix.config.json` → `"siteId"`.
